...
CATS = ["vastu","jyotish","numerology","samkhya","branding","prakriti","business","wisdom","lamafera"]
ci = 0
pa = None
webhook_set = False
...
def gen(topic, cat):
    labs = {"vastu":"MahaVastu","jyotish":"KP Jyotish","numerology":"Numerology","samkhya":"Samkhya Sutra","branding":"Branding","prakriti":"Prakriti","business":"Business Vastu","wisdom":"Vedic Wisdom","lamafera":"Lama Fera Healing"}
    lf_context = ""
    if cat == "lamafera":
        try:
            with open("lamafera-knowledge.json","r") as lkf:
                lk = json.load(lkf)
                lf_context = " REFERENCE: Origin: " + lk["origin"] + " How it works: " + lk["how_it_works"] + " Benefits include: " + ", ".join(lk["benefits_for_blogs"]["physical"][:3]) + ", " + ", ".join(lk["benefits_for_blogs"]["mental_emotional"][:3]) + ", " + ", ".join(lk["benefits_for_blogs"]["financial"][:3]) + ". SACRED BOUNDARY: " + lk["sacred_boundary"]
        except: pass
    p = 'Write 800-word blog as Acharya Mahesh Joshi (MahaVastu Acharya, KP Jyotish Scholar, Vasturang Corp LLP, Pune, guiding since 2013) on: "'+topic+'" ('+labs.get(cat,"")+'). Warm authoritative tone. Practical examples. Reference VAP methodology. No AI mention. End with CTA for askacharyamahesh.com. For Lama Fera articles: write ONLY introductory/educational content. NEVER reveal sacred symbol names, geometric forms, activation mantras, or attunement procedures. Focus on benefits, life areas where healing helps, and encourage readers to contact for sessions.' + lf_context + ' Return ONLY JSON: {"title":"SEO title","slug":"url-slug","description":"150 char","keywords":["k1","k2","k3","k4","k5"],"body_html":"<h2>..</h2><p>..</p>"}'
    try:
...
def handle(cid, text):
    ...
    if text=="/publish":
        ...
        if ok:
            ...
            arts.append({
                ...
                "cat_label":{"vastu":"MahaVastu","jyotish":"KP Jyotish","numerology":"Numerology","samkhya":"Samkhya Sutra","branding":"Branding","prakriti":"Prakriti","business":"Business Vastu","wisdom":"Vedic Wisdom","lamafera":"Lama Fera Healing"}.get(pa["category"],""),
                "desc":pa["data"]["description"],
                "keywords":pa["data"].get("keywords",[]),
                "full_content":pa["data"]["body_html"],
                "date":now_ist.strftime("%Y-%m-%d"),
                "time":now_ist.strftime("%H:%M:%S"),
                "timestamp":now_ist.isoformat(),
                "url":url,
                "published_by":"bot",
                "ist_epoch":int(now_ist.timestamp())
            })
            with open("published.json","w") as f: json.dump(arts,f,indent=2,ensure_ascii=False)
    ...
    if text.startswith("/topic "):
        ...
        elif any(w in lw for w in ["lama","fera","healing","energy","chakra","aura","tibetan","reiki","spiritual healing"]): cat="lamafera"
